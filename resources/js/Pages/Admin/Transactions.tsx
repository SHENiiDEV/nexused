...
export default function Transactions({ transactions, auditLogs }: TransactionsProps) {
    ...
    // Test webhook verification endpoint directly
    ...
    return (
        <AppLayout title="Financial Ledger & Audit Trail">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
                ...
                <div className="border-b border-slate-800 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-800/60 text-emerald-400 text-xs font-mono font-semibold uppercase mb-2">
                            <Shield className="w-3.5 h-3.5" /> HMAC-SHA256 Multi-Gateway Ledger
                        </div>
                    ...
                    {/* HMAC Webhook Simulator Buttons */}
                    <div className="flex items-center gap-2">
                        <button
                            type="button"
                            disabled={simulating}
                            onClick={() => testWebhookHmac(true)}
                            className="px-3 py-2 rounded-xl bg-emerald-950/70 border border-emerald-700 text-emerald-300 hover:bg-emerald-900 text-xs font-semibold transition-colors flex items-center gap-1.5"
                        >
                        ...
                        <button
                            ...
                            className="px-3 py-2 rounded-xl bg-rose-950/70 border border-rose-700 text-rose-300 hover:bg-rose-900 text-xs font-semibold transition-colors flex items-center gap-1.5"
                        >
                ...
                {testResult && (
                    <div className="p-4 rounded-xl bg-slate-900 border border-slate-700 font-mono text-xs text-slate-200 flex items-center gap-2">
                                    ...
                                    {transactions.data.length === 0 ? (
                                        ...
                                        transactions.data.map((txn) => (
                                            <tr key={txn.id} className="hover:bg-slate-800/40 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <div className="font-semibold text-slate-200">{txn.user?.name}</div>
                                                ...
                                                <td className="p-4 font-mono text-[10px] text-slate-400 truncate max-w-xs">
                                                    {txn.signature ? (
                                                        ...
                                                        <span className="text-slate-600">Pending Webhook</span>
                                                    )}
                                                </td>
                ...
                <div className="space-y-4 pt-6 border-t border-slate-800">
                    ...
                    <div className="rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden shadow-xl p-4">
                        <div className="space-y-2 font-mono text-xs max-h-96 overflow-y-auto">
                            {auditLogs.length === 0 ? (
                                ...
                                auditLogs.map((log) => (
                                    <div
                                        key={log.id}
                                        className="p-3 rounded-xl bg-slate-900/60 border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px]"
                                    >
